defmodule Mulberry.Document.YouTubeChannel do
  ...
  defimpl Mulberry.Document do
    ...
    @spec load(YouTubeChannel.t(), keyword()) ::
            {:ok, YouTubeChannel.t()} | {:error, any(), YouTubeChannel.t()}
    ...
    @spec transform(YouTubeChannel.t(), atom(), keyword()) ::
            {:ok, YouTubeChannel.t()} | {:error, any(), YouTubeChannel.t()}
    ...
    @spec to_chunks(YouTubeChannel.t(), keyword()) ::
    ...
    @spec to_markdown(YouTubeChannel.t(), keyword()) :: {:ok, String.t()} | {:error, any()}
    def to_markdown(%YouTubeChannel{} = channel, _opts) do
      # YouTube channels are structured data, return as markdown-formatted text
      text = build_markdown_representation(channel)
      {:ok, text}
    end

    # Private helper functions

    defp build_markdown_representation(%YouTubeChannel{} = channel) do
      parts = [
        "# #{channel.title}",
        "",
        ...
        if(channel.subscriber_count,
          do: "**Subscribers:** #{channel.subscriber_count}",
          else: nil
        ),
