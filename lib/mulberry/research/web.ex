defmodule Mulberry.Research.Web do
  ...
  defp generate_search_queries(topic, %Chain{} = chain) do
    ...
    # At least 3 queries
    num_queries = max(chain.max_sources, 3)
  ...
  defp perform_searches(queries, %Chain{} = chain, opts) do
    ...
    if length(results) < max_sources do
      ...
      additional_results =
        perform_fallback_searches_multi(
          search_modules,
      ...
      # Take more than needed for filtering
      {:ok, Enum.take(all_results, max_sources * 2)}
    else
      {:ok, results}
    end
  end
  ...
  # default to medium
  defp get_word_count_for_length(_), do: 800
  ...
  defp perform_module_searches(queries, module_config, chain, _opts) do
    ...
    |> Enum.flat_map(fn query ->
      search_and_convert_with_metadata(
        search_module,
        query,
        results_per_query,
        search_opts,
  ...
  defp perform_fallback_searches_multi(search_modules, needed, chain, opts) do
    ...
    search_modules
    # Use first 2 modules for fallback
    |> Enum.take(2)
