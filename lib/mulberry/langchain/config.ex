defmodule Mulberry.LangChain.Config do
  ...
  def get_provider(opts) do
    cond do
      ...
      provider = System.get_env("MULBERRY_LLM_PROVIDER") ->
        String.to_atom(provider)

      # 3. Check application config
      provider = Mulberry.config(:llm_provider) ->
        provider

      # 4. Auto-detect based on available API keys
      System.get_env("MULBERRY_OPENAI_API_KEY") || System.get_env("OPENAI_API_KEY") ->
        :openai

      System.get_env("MULBERRY_ANTHROPIC_API_KEY") || System.get_env("ANTHROPIC_API_KEY") ->
        :anthropic
  ...
  """
  @spec get_provider_config(provider(), operation(), Keyword.t()) ::
  ...
  def operation_defaults(:extract) do
    {:ok,
     %{
       temperature: 0.1,
       max_tokens: 4000,
  ...
  def env_config(provider) do
    ...
    config =
      ...
      |> maybe_add_env_value(
        :temperature,
        System.get_env("#{prefix}_TEMPERATURE"),
        &String.to_float/1
      )
      |> maybe_add_env_value(
        :max_tokens,
        System.get_env("#{prefix}_MAX_TOKENS"),
        &String.to_integer/1
      )
  ...
  defp maybe_add_env_value(config, key, value, transformer),
    do: Map.put(config, key, transformer.(value))
  ...
  def validate_config(provider, config)
      when provider in [:openai, :anthropic, :google, :mistral] do
    if Map.get(config, :api_key) do
      {:ok, config}
    else
      {:error,
       "API key required for #{provider}. Set #{String.upcase(Atom.to_string(provider))}_API_KEY or MULBERRY_#{String.upcase(Atom.to_string(provider))}_API_KEY environment variable."}
    end
  end
