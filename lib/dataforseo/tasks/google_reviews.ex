defmodule DataForSEO.Tasks.GoogleReviews do
  @moduledoc """
  DataForSEO task implementation for Google Reviews search.

  Fetches reviews from Google Maps for a specific business. This is an async
  task-based endpoint that requires polling for results.
  ...
  @impl true
  def task_type, do: "business_data/google/reviews"
  ...
  def parse_task_results(response) do
    require Logger
    Logger.debug("GoogleReviews parse_task_results response: #{inspect(response)}")
