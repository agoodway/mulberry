defmodule DataForSEO.TaskManager do
  @moduledoc """
  ...
  def handle_info(:check_status, %{status: :monitoring} = state) do
    # Check for timeout first, before making API calls
  ...
  @impl true
  def handle_info({:live_results, results}, state) do
    Logger.info("Live endpoint returned results immediately")
  ...
  defp create_task(state) do
    ...
    # Check if this is a live endpoint (instant results)
  ...
  defp live_endpoint?(task_module) do
    if function_exported?(task_module, :is_live_endpoint?, 0) do
      task_module.is_live_endpoint?()
    else
      false
    end
  end
    ...
    case Client.create_live_task(task_type, payload) do
      {:ok, response} ->
        ...
        case task_module.parse_task_results(response) do
          {:ok, parsed} ->
            # Send results to self to trigger completion
            send(self(), {:live_results, parsed})
            {:ok, []}
  ...
  defp handle_timeout(state) do
    reason = {:timeout, "Task timeout exceeded"}
